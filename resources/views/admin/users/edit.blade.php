...
<div class="d-flex align-items-center justify-content-between">
    ...
    <span>
        <a href="{{route('admin.users.index')}}" class="btn btn-sm btn-primary" data-toggle="tooltip" title="К списку"><i class="fa fa-arrow-left"></i></a>
        @can('delete-user')
        <form action="{{route('admin.users.destroy', $user->id)}}" class="d-inline confirmed" method="POST">
            @csrf
            <input type="hidden" name="_method" value="DELETE">
            <button href="{{route('admin.users.index')}}" class="btn btn-sm btn-danger" data-toggle="tooltip" title="Удалить"><i class="fa fa-trash"></i></button>
        </form>
        @endcan
    </span>
</div>
